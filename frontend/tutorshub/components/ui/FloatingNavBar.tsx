    ...
    <AnimatePresence mode="wait">
      ...
      >
        {navItems.map((navItem: any, idx: number) => (
          ...
          >
            <span className="text-xl lg:text-2xl">{navItem.icon}</span>
            <span className="hidden lg:inline-block font-medium text-base">
              {navItem.name}
            </span>
          </Link>
        ))}
